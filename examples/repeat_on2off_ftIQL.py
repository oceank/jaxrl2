...
def main(_):
    ...
    project_name += f"_seed{FLAGS.seed}_repeatOn2Off_oss{(FLAGS.online_stop_step//1000)}k"
    ...
    for i in tqdm.tqdm(
        ...
        if ((i-from_step) % FLAGS.eval_interval == 0) or (i == max_steps) or (i in ft_eval_steps):
            ...
            eval_info = evaluate(agent_finetunning, eval_env, num_episodes=num_episodes)
            agent_perf = eval_info['return']
            expr_now = datetime.now()
            expr_time_now_str = expr_now.strftime("%Y%m%d-%H%M%S")
            step = i
    ...
    if FLAGS.apply_best_pooling:
        ...
        if FLAGS.use_fixed_lr_for_both_offline_and_finetunning:
            ...
            kwargs_offline["decay_steps"] = FLAGS.max_offline_gradient_steps
        agent_data_collection = globals()[FLAGS.config_offline.model_constructor](
            FLAGS.seed, eval_env.observation_space.sample(), eval_env.action_space.sample(), **kwargs_offline
        )
        load_IQL_agent(orbax_checkpointer, agent_data_collection, best_finetunning_agent_filepath)
    else:
        agent_data_collection = agent_finetunning,

    ## prepare the dataset for the 2nd offline RL
    if FLAGS.online_budget_per_offline_trigger == 0:
        # use the replay buffer of the online learning as the dataset for the offline RL
        print("Online2Offline uses the online replay buffer as the dataset for the offline RL")
        experience_collection_tag=f"withbuffer{finetunning_tag}"
        dataset_offline = finetunning_replay_buffer
