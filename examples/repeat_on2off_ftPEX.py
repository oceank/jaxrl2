...
from jaxrl2.agents import SACLearner, IQLLearner, IQLBasedPEXLearner
from jaxrl2.data import ReplayBuffer, plot_episode_returns, merge_dataset_dicts
...
flags.DEFINE_integer("finetunning_budget", int(3e5), "The number of steps for a finetunning.")
...
def main(_):
    ...
    project_name += f"_seed{FLAGS.seed}_repeatOn2Off_ftPEX_oss{(FLAGS.online_stop_step//1000)}k"
    ...
    ## prepare the best online agent for the data collection of the 1st offline procedure
    if FLAGS.apply_best_pooling:
        kwargs_online = dict(FLAGS.config_online)
    ...
    else: # the use the last policy of online RL learning as the data-collection policy
        agent_data_collection = agent_online

    ## prepare the dataset for the offline RL
    if FLAGS.online_budget_per_offline_trigger == 0:
        ...
        left_budget = FLAGS.online_budget_total-FLAGS.online_stop_step 
        if FLAGS.apply_best_pooling:
            left_budget -= (int(FLAGS.eval_budget_base * FLAGS.offline_eval_budget_ratio))
        ...
        left_budget = FLAGS.online_budget_total-FLAGS.online_stop_step-new_experience_collection_budget
        if FLAGS.apply_best_pooling:
            left_budget -= (int(FLAGS.eval_budget_base * FLAGS.online_eval_budget_ratio) + int(FLAGS.eval_budget_base * FLAGS.offline_eval_budget_ratio))
    ...
    if FLAGS.save_offline_dataset:
        dataset_offline_fp = os.path.join(project_dir, f"dataset_for_offline_learning_{experience_collection_tag}.h5py")
    ...
    finetunning_budget = FLAGS.finetunning_budget
    from_step = FLAGS.online_budget_total-left_budget
    to_step = from_step+finetunning_budget
    ## Online RL validation for select the "best" policy from finetunning
    ft_eval_episodes = int(FLAGS.eval_budget_base * FLAGS.online_eval_budget_ratio / max_steps_per_episode_dict[FLAGS.env_name])
    ft_evals = ft_eval_episodes//eval_episodes_per_evaluation
    ft_eval_start_step = from_step + int(FLAGS.eval_start_step_ratio*(to_step-from_step))
    ft_eval_steps = list(
        range(
            to_step,
            ft_eval_start_step,
            -1*((to_step-ft_eval_start_step)//(ft_evals-1))
            )
        )
    if (len(ft_eval_steps) + 1) == ft_evals:
        if ft_eval_start_step == from_step: # the agent at the step 0 definitely has a randomly initialized policy. So, do not evaluate here
            ft_eval_steps.append(ft_eval_steps[-1]//2) # the step at the middle of step 0 and the the currently earliest evaluation step
        else:
            ft_eval_steps.append(ft_eval_start_step)

    if FLAGS.online_eval_budget_ratio == 0:
        ft_eval_steps = [from_step]

    ## PEX Agent Initialization
    ### Reload the output policy from the preceding offline phase
    if FLAGS.apply_best_pooling:
        kwargs_offline = dict(FLAGS.config_offline.model_config)
        if FLAGS.use_fixed_lr_for_both_offline_and_finetunning:
            kwargs_offline.pop("cosine_decay", False)
        ...
        load_IQL_agent(orbax_checkpointer, agent_offline, best_offline_agent_filepath)

    ### Initialize the online agent using IQL learner
    kwargs_offline = dict(FLAGS.config_offline.model_config)
    if FLAGS.use_fixed_lr_for_both_offline_and_finetunning:
        kwargs_offline.pop("cosine_decay", False)
        kwargs_offline["decay_steps"] = None
        ...
        kwargs_offline["decay_steps"] = finetunning_budget
    ...
    ### Initialize online and offline buffers
    pex_online_replay_buffer = ReplayBuffer(
        env.observation_space, env.action_space, FLAGS.online_budget_total
    )
    pex_online_replay_buffer.seed(FLAGS.seed)
    pex_offline_replay_buffer = dataset_offline
    ...
    finetunning_tag = f"from{from_step//1000}kto{to_step//1000}k"
    ...
    bp_finetunning_eval_filepath = f"{project_dir}/eval_ave_episode_return_finetunning{finetunning_tag}_bp.txt"
    with open(bp_finetunning_eval_filepath, "w") as f:
        f.write(f"Experiment Time\tStep\tReturn\n")

    bp_finetunning_eval_file = open(bp_finetunning_eval_filepath, "a")


    start_step = from_step+1
    max_steps = to_step #FLAGS.online_budget_total
    best_finetunning_agent_step = None
    best_finetunning_agent_perf = None
    best_finetunning_agent_filepath = f"{project_dir}/ckpts/best_finetunned_agent_{finetunning_tag}"
    observation, done = env.reset(), False
    for i in tqdm.tqdm(
        range(start_step, max_steps + 1), smoothing=0.1, disable=not FLAGS.tqdm
    ):
        start_training = len(pex_online_replay_buffer) >= FLAGS.start_online_training
        ...
        pex_online_replay_buffer.insert(
            dict(
                observations=observation,
                actions=action,
                rewards=reward,
        ...
        if start_training:
            ...
            batch_offline_buffer = pex_offline_replay_buffer.sample(batch_size_offline_buffer)
            batch_online_buffer = pex_online_replay_buffer.sample(batch_size_online_buffer)
        ...
        if ((i-from_step) % FLAGS.eval_interval == 0) or (i == max_steps) or (i in ft_eval_steps):
            if i in ft_eval_steps:
                num_episodes = eval_episodes_per_evaluation 
            else:
                num_episodes = FLAGS.eval_episodes
            eval_info = evaluate(agent_pex, eval_env, num_episodes=num_episodes)
            agent_perf = eval_info['return']
            expr_now = datetime.now()
            expr_time_now_str = expr_now.strftime("%Y%m%d-%H%M%S")
            step = i
            ...
            if i in ft_eval_steps:
                bp_finetunning_eval_file.write(f"{expr_time_now_str}\t{step}\t{agent_perf}\n")
                bp_finetunning_eval_file.flush()
                save_log(summary_writer, eval_info, i, f"evaluation-finetunning{finetunning_tag}_bp", use_wandb=FLAGS.wandb)
                ...
                if best_finetunning_agent_perf is None or best_finetunning_agent_perf < agent_perf:
                    ...
                    save_agent(orbax_checkpointer, agent_pex, i, best_finetunning_agent_filepath)

                # log the best model performance so far
                save_log(summary_writer, {"best_ckpt_return":best_finetunning_agent_perf}, i, "evaluation", use_wandb=FLAGS.wandb)


    bp_finetunning_eval_file.write(f"**Best Policy**\t{best_finetunning_agent_step}\t{best_finetunning_agent_perf}\n")
    bp_finetunning_eval_file.close()
    finetunning_eval_file.close()


    # Phase 4: the 2nd offline RL phase (the last phase)
    ## prepare the best online agent for the data collection of the 2nd offline phase 
    if FLAGS.apply_best_pooling:
        agent_data_collection = IQLBasedPEXLearner(FLAGS.seed, agent_online, agent_offline, inv_temperature, transfer_critic, copy_to_target)
        load_agent(orbax_checkpointer, agent_data_collection, best_finetunning_agent_filepath)
    else:
        agent_data_collection = agent_pex,

    ## prepare the dataset for the offline RL
    if FLAGS.online_budget_per_offline_trigger == 0:
        # use the replay buffer of the online learning as the dataset for the offline RL
        print("Online2Offline uses the online replay buffer as the dataset for the offline RL")
        experience_collection_tag=f"withbuffer{finetunning_tag}"
        dataset_offline = pex_online_replay_buffer # another option is the union of pex_online_replay_buffer and pex_offline_replay_buffer
        consumed_budget = to_step
        left_budget = FLAGS.online_budget_total-to_step
        if FLAGS.apply_best_pooling:
            left_budget -= (int(FLAGS.eval_budget_base * FLAGS.offline_eval_budget_ratio))
            consumed_budget += (int(FLAGS.eval_budget_base * FLAGS.offline_eval_budget_ratio))
    else:
        ...
        eps_rng = jax.random.PRNGKey(FLAGS.seed)
        new_experience_collection_budget = FLAGS.online_budget_per_offline_trigger 
        prev_consumed_budget = to_step
        consumed_budget = prev_consumed_budget + new_experience_collection_budget
        left_budget = FLAGS.online_budget_total-consumed_budget
        if FLAGS.apply_best_pooling:
            left_budget -= (int(FLAGS.eval_budget_base * FLAGS.online_eval_budget_ratio) + int(FLAGS.eval_budget_base * FLAGS.offline_eval_budget_ratio))
            consumed_budget += (int(FLAGS.eval_budget_base * FLAGS.online_eval_budget_ratio) + int(FLAGS.eval_budget_base * FLAGS.offline_eval_budget_ratio))
            prev_consumed_budget += int(FLAGS.eval_budget_base * FLAGS.online_eval_budget_ratio)
